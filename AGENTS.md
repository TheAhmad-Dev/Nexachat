...
- Unit-testing Expo code headlessly: react-native's Flow source can't be PARSED by esbuild under Node, so post-load patching can't work — stub modules with a `Module._load` hook installed BEFORE the import (see `services/mediaService.unit.test.ts`). Also needed: `(globalThis as any).__DEV__ = false`, and wrap async suites in `main()` (tsx emits CJS; top-level await fails). `*.test.ts` files are excluded from the app tsconfig (Metro/Babel is the real compiler); run them with `npx tsx <file>`.
